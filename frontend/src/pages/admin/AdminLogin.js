...
export default function AdminLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Invalid credentials. Default: admin / admin123');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="auth-page">
      <div className="auth-card fade-up">
        ...
        <div style={{ marginTop:16, padding:'12px 14px', background:'rgba(255,255,255,0.02)', border:'1px solid var(--border)', borderRadius:9, fontSize:'.78rem', color:'var(--muted)', fontFamily:'var(--font-mono)' }}>
          Default credentials: <span style={{ color:'var(--eth)' }}>admin</span> / <span style={{ color:'var(--eth)' }}>admin123</span>
        </div>
      </div>
    </div>
  );
