...
export default function UserSignup() {
  ...
  const handleVerify = async () => {
    ...
    try {
      await axios.post('/api/auth/user/verify-email', { email: form.email, code });
  ...
  const resend = async () => {
    try {
      const res = await axios.post('/api/auth/user/resend-otp', { email: form.email });
      setInfo(res.data.message);
      if (res.data.devCode) setDevCode(res.data.devCode);
      setOtp(['', '', '', '', '', '']);
  ...
  const pwStrength = p => {
    ...
    if (p.length < 6)  return { w: '30%', c: 'var(--red)', l: 'Too short' };
    if (p.length < 10) return { w: '65%', c: 'var(--amber)', l: 'Fair' };
          ...
          <>
            ...
            <form onSubmit={handleRegister}>
              <div className="form-group">
                ...
                <input className="form-control" type="text" placeholder="Your legal full name" value={form.name} onChange={e=>setForm({...form,name:e.target.value})} required autoFocus />
              </div>
              <div className="form-group">
                ...
                <div className="form-hint">A verification code will be sent here</div>
              </div>
              <div className="form-group">
                <label className="form-label">Phone Number</label>
                <input className="form-control" type="tel" placeholder="+977 980 000 0000" value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} />
              </div>
              <div className="form-group">
              ...
              <div className="form-group">
                ...
                <input className="form-control" type="password" placeholder="Re-enter password" value={form.confirm} onChange={e=>setForm({...form,confirm:e.target.value})} required />
              ...
              <button type="submit" className={`btn btn-primary btn-full btn-lg ${loading?'btn-loading':''}`} disabled={loading}>
                {loading ? 'Creating account…' : '🗳️ Create Account & Send Code'}
              </button>
            </form>
            <div className="auth-footer">Already have an account? <Link to="/user/login">Sign In →</Link></div>
            <div style={{textAlign:'center',marginTop:8}}><Link to="/" style={{fontSize:'.82rem',color:'var(--muted)'}}>← Home</Link></div>
          </>
        ) : (
          <>
            <div style={{textAlign:'center',marginBottom:24}}>
              ...
              <div className="auth-sub">We sent a 6-digit code to <strong style={{color:'var(--eth)'}}>{form.email}</strong></div>
            </div>
            ...
            <div style={{display:'flex',gap:10,justifyContent:'center',marginBottom:24}} onPaste={handleOtpPaste}>
              {otp.map((d, i) => (
                <input
                  ...
                  style={{
                    width:52, height:58, textAlign:'center', fontSize:'1.6rem', fontWeight:700,
                    background:'var(--card2)', border:`2px solid ${d ? 'var(--eth)' : 'var(--border2)'}`,
                    borderRadius:12, color:'var(--text)', outline:'none', fontFamily:'var(--font-mono)',
            ...
            <button onClick={handleVerify} className={`btn btn-primary btn-full btn-lg ${loading?'btn-loading':''}`} disabled={loading || otp.join('').length < 6}>
